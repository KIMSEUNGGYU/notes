...
│       └── content/
│           ├── study/{주제}/      # 학습 — live 에서 통째로 제외
│           │   ├── 개념/           # 활용하기 위해 알아야 하는 것
│           │   └── 활용/           # 그 개념을 실전에 어떻게 활용했나
...
- **wiki** — 경로·파일명 규칙. `content/study/` 전체와 `*.draft.md` 가 `srcExclude` 로 빠져서 페이지 자체가 안 만들어집니다.

wiki는 사이드바가 자동 생성이라 배열에 플래그를 달 자리가 없어서 규칙으로 가릅니다.

### wiki study 문서 배치

주제마다 두 축으로 가릅니다. 가르는 질문은 **우리 코드·이슈번호·실측 없이도 읽히나** 하나입니다.

```
study/{주제}/
├── 개념/         활용하기 위해 알아야 하는 것 (읽힌다)
└── 활용/         그 개념을 실전에 어떻게 활용했나 (우리 것이 있어야 말이 된다)
```

사이드바가 폴더 스캔으로 자동 생성되므로 목차용 `index.md` 는 두지 않습니다 — 같은 목록이 두 곳이 됩니다.

둘 다 계속 갱신되므로 **날짜 접두사를 붙이지 않습니다.** 언제 썼는지는 git 이력이 답합니다.
`활용/` 문서는 주어가 날짜가 아니라 개념이라, `개념/` 문서의 절과 마주 보게 씁니다.
