---
title: Claude Code 플러그인 — 4가지 컴포넌트 이해하기
description: Skill·Command·Agent·Hook — 플러그인을 이루는 4가지 컴포넌트와 실제 설정 예시 (2026-02-09)
head:
  - - meta
    - name: keywords
      content: claude-code, plugin, skill, command, agent, hook
---

# Claude Code 플러그인 — 4가지 컴포넌트 이해하기

발표일: 2026년 2월 9일

## 플러그인이란?

Claude Code에 기능을 추가하는 설정 묶음. 4개 컴포넌트로 구성됩니다.
...
### 실제 설정: `skills/code-review/SKILL.md`

```yaml
---
name: code-review
description: Run a comprehensive code review, FE 철학에 맞게 코드리뷰를 수행한다. 코드리뷰 요청, codereview 등이 온 경우 수행한다.
[속성]: true
---
```
...
::: warning
Skill은 "뭘 봐야 하는지"만 알려줌. "어떤 순서로 할지"는 Claude 자유 → 품질이 불안정할 수 있음.
:::
...
::: tip
Command를 호출하면 Skill도 함께 로드된다. 컨텍스트에 "code review", "architecture" 등의 키워드가 포함되면 해당 Skill이 자동으로 기준을 주입한다. → **Command(절차) + Skill(기준)이 항상 같이 동작**하는 구조.
:::
...
### 실제 설정: `scripts/pre-tool-enforcer.mjs` (핵심 발췌)
